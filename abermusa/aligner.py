...
import glob, os, cPickle, math, scoring, traceback
...
def handleReference(args,ref,exe,logf):
    ...
    # Preparation steps.
    rf = IO.getFileName(ref)      # File name.
    if not handleFolder(rf,logf): return False
    ...
    for f in fl:
        ...
        trbf = '%s.dump'  %   (pr) # Traceback file in case of scoring failure.
        ...
        if os.path.isfile(scfi): scored.append((name,scfi))               # Scored already.
            ...
            # Output (stdout) was generated by program but nothing else; probably failed.
            failed.append(name)
            fdetails.append('Failed scoring %s (external aligner failed - missing <%s>).' % (
                name,f))
            ...
            try: os.remove(outf) # Removes its stdout file if succeeded.
                ...
                logf.write('WARNING; Could not remove external aligner stdout file %s.' % (outf))
